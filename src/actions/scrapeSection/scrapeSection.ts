...
export default async function scrapeSection(
  ...
  for (let i = 0; i < initialLessonHandles.length; i++) {
    ...
    const nextName = await currentHandle.evaluate((el) => {
      console.log(el);
  ...
  console.log('so what happens here?');
  console.log(initialUrl);
  console.log(lessonNames);

  for (let i = 0; i < lessonNames.length; i++) {
    console.log('steps into this loop ' + i);
    ...
    console.log('SUCKSSSSSSSSSSSSSSSSSSSSSSSSSSSSS');
    // navigate to lesson
    try {
