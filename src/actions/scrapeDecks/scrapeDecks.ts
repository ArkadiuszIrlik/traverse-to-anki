...
function scrapeCurrentDeck(page: Page) {
  logger.info(`Currently scraping ${page.url()}`);
}
  ...
  */
  ...
  for (let i = 0; i < decksToScrape.length; i++) {
    ...
    try {
      ...
      logger.error(`Couldn't navigate to ${currentDeck}. Error: ${err}`);
